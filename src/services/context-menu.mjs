...
import { addToCollection, deleteFromCollection, getCollections } from './collections.mjs';
import prompt from 'electron-prompt';
import { openCollection, updateCollection } from '../views/collection/lib.mjs';
...
async function getCollectionsMenu (filePath) {
	...
	return cols.map(x => ({
		label: x,
		click: async () => {
			await addToCollection(x, filePath);
			updateCollection(x);
		}
	}));
}
...
export async function init () {
	...
	ipcMain.handle('context-menu-col', async (e, col, filePath, thumbnailSize = 0) => {
		const sender = e.sender;
		const win = BrowserWindow.fromWebContents(sender);
		...
		const template = [
			...
			),
			{
				...
				click: async () => {
					await deleteFromCollection(col, filePath);
					openCollection(col);
				}
			},
		];
